...
author: Zien Gao
thanks: "Code and data supporting this analysis are available at: <https://github.com/lauragao75/Understanding-the-Ratings-of-Children-s-Books-A-Data-Driven-Analysis>"
date: today
date-format: long
...
We use the statistical programming language R [@R] to generate, clean, and analyze the dataset. Our data, sourced from the children's book ratings dataset hosted on GitHub by Timothy A. Cookson [@dataset], includes metadata for 9,240 children's books. After cleaning, 3,769 children’s books are used to analyze. The dataset contains information like International Standard Book Number, title, author, cover type, pages, and rating, which form the basis for our analysis. Children's book ratings offer insights into reader preferences, guiding publishers and authors in producing appealing literature. This dataset provides a valuable lens into these preferences through user-generated ratings. We focus on attributes that may influence book ratings, such as the type of cover, number of pages, and the original year of publishing.

The libraries used in our analysis include `tidyverse` [@tidyverse], `arrow` [@arrow], `broom` [@broom], `ggplot2` [@ggplot2], `knitr` [@knitr], `kableExtra` [@kableExtra], `here` [@here], `bayesplot` [@bayesplot], `broom.mixed` [@broommixed], `coda` [@coda] and `modelsummary` [@modelsummary]. These libraries enable data manipulation, model building, and visualization, ensuring a comprehensive examination of the dataset to explore relationships between book characteristics and ratings.
...
To provide context for the analysis, we present some key summary statistics of the dataset. @tbl-summary-table includes the average rating, distribution of book types, and the breakdown of page ranges and publication periods.

```{r}
#| label: tbl-summary-table
#| echo: false
#| tbl-cap: "Summary statistics for the dataset of 3,769 children’s books, showing that hardcover books are slightly more common than paperbacks. Most books are under 50 pages, and those published after 2000 account for 44% of the dataset. The average rating is 4.04, reflecting general reader satisfaction."
...
# Function to create count summaries
create_count_summary <- function(data, count_column, prefix) {
  data %>%
    count({{ count_column }}) %>%
    rename(Metric = {{ count_column }}, Value = n) %>%
    mutate(Metric = paste(prefix, Metric, sep = ": "))
}
...
# Page range and publication period counts
pages_range_count <- create_count_summary(analysis_data, pages_range, "Page Range")
publish_period_count <- create_count_summary(analysis_data, publish_period, "Publication Period")

# Combine summary statistics
combined_summary <- bind_rows(summary_stats, pages_range_count, publish_period_count)
...
#| fig-cap: "Most children's books receive ratings between 3.5 and 4.5, with a median rating of 4.0."
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: "Hardcover books tend to receive higher ratings than paperbacks, with an average rating of 4.2 compared to 4.0."
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: "Books with 150-299 pages have the highest average ratings (4.3), followed by books with 300-499 pages (4.2)."
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: "Books published after 2000 have the highest average ratings (4.3), compared to books from earlier periods (3.8–4.1)."
#| echo: false
#| warning: false
#| message: false
...
\mu_i &= \alpha + \eta_1(\text{CoverType}_i) + \eta_2(\text{PageRange}_i) + \eta_3(\text{PublicationPeriod}_i) \\
\alpha &\sim \text{Normal}(0, 2.5) \\
\eta_1 &\sim \text{Normal}(0, 2.5) \\
\eta_2 &\sim \text{Normal}(0, 2.5) \\
\eta_3 &\sim \text{Normal}(0, 2.5) \\
...
@tbl-regression-results summarizes the regression results, showing the estimated coefficients, median absolute deviations, and credible intervals for each predictor. Credible intervals confirm that these associations are statistically significant.
```{r}
#| label: tbl-regression-results
#| echo: false
#| tbl-cap: "Regression model summary showing that hardcover books and those with 150-299 pages are positively associated with higher ratings, while books published before 2000 tend to receive lower ratings."
#| warning: false
#| message: false
...
Another convergence diagnostic involves examining trace plots. Trace plots show the sampled values of each parameter across iterations for each Markov chain. Well-mixed chains with no clear trends suggest that the model has converged. In @fig-trace-plot, we observe the trace plots for key parameters in the model. The lack of discernible patterns or trends confirms that convergence has been reached.

```{r}
#| label: fig-trace-plot
#| echo: false
#| fig-cap: "Trace plots for key parameters across the Markov chains, showing well-mixed chains with stable values."
#| fig-width: 10 
#| fig-height: 7 
#| dpi: 300
...
Posterior predictive checks are used to assess the goodness-of-fit of the Bayesian model. By simulating data from the posterior distribution, we can compare the simulated data to the observed data to determine if the model adequately represents the data. @fig-posterior-checks illustrates the results of posterior predictive checks for the Bayesian model. This helps evaluate whether the model adequately captures the key features of the observed data.

```{r}
#| label: fig-posterior-checks
#| echo: false
#| fig-cap: "Posterior predictive checks showing a comparison between the observed data and simulated data generated from the model's posterior distribution."
...
@tbl-posterior-sum is a summary table of the key parameters in the model, including the estimated means, standard deviations, and 95% credible intervals.

```{r}
#| label: tbl-posterior-sum
#| echo: false
#| tbl-cap: "Posterior summary statistics for key model parameters, including means, standard deviations, and 95% credible intervals. Hardcover books and mid-length page ranges (150–299 pages) show positive associations with higher ratings, with tight credible intervals confirming robust estimates."
#| warning: false
#| message: false
...
# Summarize the posterior distributions into a tidy data frame
posterior_summary <- broom.mixed::tidyMCMC(
  as.matrix(children_model), 
  conf.int = TRUE, 
  conf.level = 0.95
)
...
Posterior density plots provide a visual representation of the uncertainty in the model parameters. @fig-results-effects show the distribution of possible values for each parameter, which helps in understanding the spread and central tendency.

```{r}
#| label: fig-results-effects
#| echo: false
#| fig-cap: "Posterior density plots for key model parameters show the range and central tendency of plausible values. Parameters such as hardcover books and mid-length page ranges demonstrate a positive association with higher ratings, supported by narrow, well-defined density curves."
#| warning: false
#| fig-width: 10 
#| fig-height: 7 
#| dpi: 300
...
## Data Collection Challenges and Bias
